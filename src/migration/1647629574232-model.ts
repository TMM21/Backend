...
export class model1647629574232 implements MigrationInterface {
    name = 'model1647629574232'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`users\` (\`id\` varchar(36) NOT NULL, \`name\` varchar(255) NOT NULL, \`phoneNumber\` varchar(255) NOT NULL, \`email\` varchar(255) NOT NULL, \`password\` varchar(255) NOT NULL, \`gender\` varchar(255) NOT NULL, \`priviledges\` text NOT NULL, \`refreshToken\` varchar(255) NULL, \`Terminal\` varchar(255) NULL, \`TerminalName\` varchar(255) NULL, \`block\` tinyint NOT NULL DEFAULT 0, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`rolesId\` varchar(36) NULL, \`priviledgeId\` varchar(36) NULL, UNIQUE INDEX \`IDX_1e3d0240b49c40521aaeb95329\` (\`phoneNumber\`), UNIQUE INDEX \`IDX_97672ac88f789774dd47f7c8be\` (\`email\`), UNIQUE INDEX \`IDX_4fdf5f552fcfe06082a35e9728\` (\`refreshToken\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`vehicles\` (\`id\` varchar(36) NOT NULL, \`plateNumber\` varchar(255) NOT NULL, \`chasisNumber\` varchar(255) NOT NULL, \`PC\` varchar(255) NOT NULL, \`PCPhoneNumber\` varchar(255) NOT NULL, \`PCNextOfKin\` varchar(255) NOT NULL, \`PCNextOfKinPhoneNumber\` varchar(255) NOT NULL, \`HC\` varchar(255) NULL, \`HCNextOfKin\` varchar(255) NULL, \`HCNextOfKinPhoneNumber\` int NULL, \`Location\` varchar(255) NOT NULL, \`vehicleStatus\` enum ('available', 'in_transit', 'assigned', 'arrived', 'down') NOT NULL DEFAULT 'available', \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`vehicleTypeId\` varchar(36) NULL, UNIQUE INDEX \`IDX_66ea96381a7a7ceb35c72f3662\` (\`plateNumber\`), UNIQUE INDEX \`IDX_bf694919e3c7689ffaa0d880b1\` (\`chasisNumber\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`vehicle_type\` (\`id\` varchar(36) NOT NULL, \`seatNumber\` int NOT NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`featureId\` varchar(36) NULL, \`vehicleId\` varchar(36) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`captain_fee\` (\`id\` varchar(36) NOT NULL, \`NameOfCaptain\` varchar(255) NOT NULL, \`Route\` varchar(255) NOT NULL, \`fee\` int NOT NULL, \`TravelDate\` varchar(255) NOT NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`vehicle_location\` (\`id\` varchar(36) NOT NULL, \`plateNumber\` varchar(255) NOT NULL, \`Location\` varchar(255) NOT NULL, \`Left\` varchar(255) NOT NULL, \`PC\` varchar(255) NOT NULL, \`HeadingTo\` varchar(255) NOT NULL, \`status\` enum ('available', 'in_transit', 'assigned', 'arrived', 'down') NOT NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`vehicle_type\` ADD CONSTRAINT \`FK_6bff1d4b89b6a00ccb87463ddd1\` FOREIGN KEY (\`vehicleId\`) REFERENCES \`vehicles\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`vehicle_type\` DROP FOREIGN KEY \`FK_6bff1d4b89b6a00ccb87463ddd1\``);
