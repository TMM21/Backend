...
export class VehicleService {
  public createVehicle = async (vehicleData: AddVehicle) => {
    const { isValidPhoneNumber, parsedPhoneNumber } = this.parsePhoneNumber(
      vehicleData.PCPhoneNumber,
    )
    ...
    try {
      ...
      if (vehicle) {
        throw new AppError(
          `The PC ${vehicleData.PC} is already attributed to a vehicle`,
        )
      }

      const type = await VehicleType.findOneOrFail({
        id: vehicleData.typeId,
      }).catch(() => {
