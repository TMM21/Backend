...
export class AuthService {
  ...
  createCustomer = async (customer: addCustomer) => {
    ...
    let { parsedPhoneNumber, isValidPhoneNumber } = this.parsePhoneNumber(
    ...
    if (phone) {
      throw new AppError(
        `An account with ${customer.phoneNumber} already exists`,
      )
    }
    ...
    if (email) {
      throw new AppError(
        `A user with this email ${customer.email} already exists `,
      )
    }
    //verify role
    const role = await Roles.findOneOrFail({
      where: [{ role: customer.role }],
    }).catch(() => {
      throw new AppError('invalid role selected')
    })
    const expriviledge = await Priviledge.findOne({
  ...
  public loginCustomer = async (loginData: login) => {
    ...
    let { isValidPhoneNumber, parsedPhoneNumber } = this.parsePhoneNumber(
  ...
  public createUser = async (addUser: addCustomer) => {
    ...
    if (phone) {
      throw new AppError(
        `A user with this phoneNumber ${addUser.phoneNumber} already exist`,
      )
    }
    const email = await Users.findOne({ where: [{ email: addUser.email }] })
    if (email) {
      throw new AppError(
        `A user with this email ${addUser.email} already exists`,
      )
    }
    ...
    const priviledge = await Priviledge.findOne({
      where: [{ name: role.role }],
    })
  ...
  public AssignPriviledge = async (id: string, assign: assignPriviledge) => {
    const user = await Users.findOneOrFail({ where: [{ id }] }).catch(() => {
      throw new AppError('user not found')
    })

    for (let priviledge of assign.priviledge) {
      const isMatch = user.priviledges.every((item) =>
        priviledge.includes(item),
      )
      if (!isMatch) {
  ...
  private parsePhoneNumber(phoneNumber: string) {
    ...
    try {
      googlePhoneNumber = phoneNumberUtilInstance.parse(
        phoneNumber.toString(),
        'NG',
      )
      parsedPhoneNumber = phoneNumberUtilInstance.format(
        googlePhoneNumber,
    ...
    const isValidPhoneNumber = phoneNumberUtilInstance.isValidNumberForRegion(
      googlePhoneNumber,
      'NG',
    )
  ...
  private generateAcessToken = async (user: Users) => {
    ...
    const token = jwt.sign(
      { iss: 'http://localhost:3000', user: body },
      'AIzaSyDnN72_PIUPd6mHgVQv2GuhpLn4wot3ke4',
      {
        expiresIn: '1d',
      },
    )
