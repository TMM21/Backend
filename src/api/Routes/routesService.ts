...
export class RoutesService {
  public createRoutes = async (routeData: AddRoutes) => {
    ...
    const exroute = await Routes.findOne({
      where: [
        {
    ...
    if (terminal.name == routeData.arrivalName) {
      throw new AppError(
        'departureTerminal and arrivalTerminal cannot be the same',
      )
    }
    console.log(terminal.name, routeData.arrivalName)
