...
export class BookingService {
      ...
      where: [
        {
          ...
          vehicleType: type.id,
          vehicleStatus: Not(VehicleStatus.IN_TRANSIT),
        },
      ],
    })
        ...
        where: [
          {
            ...
            bookingStatus: BookingStatus.DELAY,
          },
        ],
      }).catch(() => {
        throw new AppError('no referenceId match this search')
      })
  ...
  public Reference = async (reference: refernce, user: Users) => {
    ...
    const ref = await Bookings.findOneOrFail({
  ...
  public PrintManifest = async (manifest: manifest, user: Users) => {
    ...
    try {
      const print = await Bookings.find({
        where: [
          {
